...
class Transformer(nn.Module):
    def __init__(self, depth, query_size, key_size, value_size, emb_size, num_heads, channels, expansion, device,
        ...
        self.temporal_mask = [channels[0], channels[2]]
        self.spatial_mask = [channels[1], channels[3]]
        ...
        self.eeg_spatial_cross_encoder = TransformerCrossEncoder(depth, query_size, key_size, value_size, emb_size,
                                                              num_heads, None, expansion, cross_dropout, device)
        self.nirs_spatial_cross_encoder = TransformerCrossEncoder(depth, query_size, key_size, value_size, emb_size,
                                                                  num_heads, None, expansion, cross_dropout, device)

    def forward(self, temporal_eeg, temporal_nirs, spatial_eeg, spatial_nirs):

        spatial_context = self.spatial_context(spatial_eeg, spatial_nirs, self.spatial_mask)
        temporal_context = self.temporal_context(temporal_eeg, temporal_nirs, self.temporal_mask)

        eeg_temporal_cross_outputs = self.eeg_temporal_cross_encoder(temporal_eeg, temporal_context)
        nirs_temporal_cross_outputs = self.nirs_temporal_cross_encoder(temporal_nirs, temporal_context)

        eeg_spatial_cross_outputs = self.eeg_spatial_cross_encoder(spatial_eeg, spatial_context)
        nirs_spatial_cross_outputs = self.nirs_spatial_cross_encoder(spatial_nirs, spatial_context)

        eeg_spatial_outputs = self.eeg_spatial_encoder(eeg_spatial_cross_outputs)
        nirs_spatial_outputs = self.nirs_spatial_encoder(nirs_spatial_cross_outputs)

        eeg_temporal_outputs = self.eeg_temporal_encoder(eeg_temporal_cross_outputs)
        nirs_temporal_outputs = self.nirs_temporal_encoder(nirs_temporal_cross_outputs)
...
class ClassificationHead(nn.Module):
    ...
    def forward(self, out):
        ...
        eeg_outputs = self.eeg(eeg_outputs) * w1
        nirs_outputs = self.nirs(nirs_outputs) * w2

        out = eeg_outputs + nirs_outputs
        return out
