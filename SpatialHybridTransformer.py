...
class Transformer(nn.Module):
    ...
    def forward(self, temporal_eeg, temporal_nirs, spatial_eeg, spatial_nirs):
        # # eeg spatial self transformer
        eeg_spatial_outputs = self.eeg_spatial_encoder(spatial_eeg)

        nirs_spatial_outputs = self.nirs_spatial_encoder(spatial_nirs)

        spatial_context = self.spatial_context(eeg_spatial_outputs, nirs_spatial_outputs, self.mask)
        ...
        return [eeg_spatial_outputs[:, 0], nirs_spatial_outputs[:, 0],
...
class ClassificationHead(nn.Module):
    def __init__(self, num_classes, emb_size, dropout):
        ...
        self.eeg_spatial = nn.Sequential(
            nn.Linear(emb_size * 1, num_classes),
        )
        self.nirs_spatial = nn.Sequential(
            nn.Linear(emb_size * 1, num_classes)
        )
        self.fusion_spatial = nn.Sequential(
            nn.Linear(emb_size * 1, num_classes)
        )
        self.w = nn.Parameter(torch.Tensor([1., 1., 0.5]), requires_grad=True)

    def forward(self, out):
        eeg_spatial, nirs_spatial, spatial_cross = out
        spatial_cross = self.attention_weight_sum(spatial_cross)
        w1 = torch.exp(self.w[0]) / torch.sum(torch.exp(self.w))
        w2 = torch.exp(self.w[1]) / torch.sum(torch.exp(self.w))
        w3 = torch.exp(self.w[2]) / torch.sum(torch.exp(self.w))
        eeg_spatial, nirs_spatial, cross_spatial = self.eeg_spatial(eeg_spatial) * w1, self.nirs_spatial(
            nirs_spatial) * w2, self.fusion_spatial(spatial_cross) * w3
        # eeg_spatial, cross_spatial = self.uni_modality(eeg_spatial) * w1, self.cross_modality(spatial_cross) * w2
        out = eeg_spatial + nirs_spatial + cross_spatial
...
class HybridTransformer(nn.Module):
    ...
    def forward(self, eeg, nirs):
        ...
        out1 = self.transformer(spatial_eeg, spatial_nirs, spatial_eeg, spatial_nirs)
